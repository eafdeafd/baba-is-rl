...
from baba_is_ai.baba.registration import make
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor = nn.Sequential(
            layer_init(nn.Conv2d(input_channels, 32, 4, stride=2)),
            nn.ReLU(),
    ...
    def get_value(self, x):
        return self.critic(preprocess(x))

    def get_action_and_value(self, x, action=None):
        logits = self.actor(preprocess(x))
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(preprocess(x))
