...
#        idle = 0
    ...
    def __init__(
            ...
            randomize_baba: bool = False,
            randomize_obj: bool = True,
            **kwargs,
    ):
        self.randomize_baba= randomize_baba
        ...
        # set render mode bc they didn't?
        assert render_mode in self.metadata["render_modes"], f"Unsupported render mode: {mode}"
    ...
    def render(self, mode="human", highlight=True, tile_size=TILE_PIXELS):
        assert mode in self.metadata["render_modes"], f"Unsupported render mode: {mode}"
        ...
        if mode == "human":
            #print("Showing image in window")  # Debug print
            self.window.show_img(img)
        elif mode in ["rgb_array", "single_rgb_array"]:
            #print(f"Returning image for mode: {mode}")  # Debug print
            return img
        else:
            print(f"Unhandled mode: {mode}")  # Debug print
        
        print("Render method reached end without returning")  # Debug print
        return None  # Explicit return of None if no other return is reached
