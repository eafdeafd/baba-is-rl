...
class Args:
    ...
    wandb_project_name: str = "rl-is-baba"
    """the wandb's project name"""
    wandb_entity: str = "amwu"
...
def preprocess(x):
    x = x.float() / 20.0 # normalize by number of objects (this is jank)
    x = einops.rearrange(x, 'b w h c -> b c h w')  # Change shape to (batch_size, channels, height, width)
    x = x[:, 0, :, :].unsqueeze(1) # ignore channels besides objects (color, status doesn't matter for now)
    return x
...
if __name__ == '__main__':
    ...
    q_network = QCNNetwork(envs).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate)
    target_network = QCNNetwork(envs).to(device)
